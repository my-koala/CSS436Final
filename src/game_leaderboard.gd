...
const GAME_LEADERBOARD_ENTRY_SCENE: PackedScene = preload("uid://cstqcudoy2wth")

@onready
var _game_data: GameData = $"../../../../../../game_data" as GameData
var _game_data_dirty: bool = false
@onready
var _container_entries: VBoxContainer = $scroll_container/container_entries as VBoxContainer
...
func _refresh_entries() -> void:
	...
	for player_id: int in player_ids:
		...
		if _entries.has(player_id):
			...
			entry = GAME_LEADERBOARD_ENTRY_SCENE.instantiate() as GameLeaderboardEntry
			_container_entries.add_child(entry)
		...
		_container_entries.move_child(entry, _game_data.get_player_place(player_id))
